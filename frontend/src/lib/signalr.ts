...
export function startConnection(onJobCompleted?: (data: { jobId: string; preset: string; message: string }) => void) {
  ...
  try {
    connection = new signalR.HubConnectionBuilder()
      ...
      .configureLogging(signalR.LogLevel.None)  // Suppress console errors
      .build();
    ...
    connection.start().catch(() => {
      // SignalR is optional — dashboard falls back to polling
    });
  } catch {
    // SignalR setup failed — silently ignore, polling will handle updates
  }
}
